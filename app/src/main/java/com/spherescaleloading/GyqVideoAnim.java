...
public class GyqVideoAnim extends View {
	...
	public GyqVideoAnim(Context context) {
		super(context);
	}
	public GyqVideoAnim(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.gyq_video_anim);
		circleRadius = (int)a.getDimension(R.styleable.gyq_video_anim_circleRadius, paramsCreator.getDefaultCircleRadius());
		circleSpacing = (int)a.getDimension(R.styleable.gyq_video_anim_circleSpacing, paramsCreator.getDefaultCircleSpacing());
		int cycle = a.getInt(R.styleable.gyq_video_anim_cycle, 2000);//周期，默认为2秒
		cycle = cycle/2;
